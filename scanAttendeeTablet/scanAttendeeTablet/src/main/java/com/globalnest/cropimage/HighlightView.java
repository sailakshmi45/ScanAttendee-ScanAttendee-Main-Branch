    ...
    protected void draw(Canvas canvas) {
        ...
        if (!hasFocus()) {
            ...
            if (mCircle) {
                ...
                Rect topRect =
                    new Rect(viewDrawingRect.left, viewDrawingRect.top, viewDrawingRect.right,
                        mDrawRect.top);
                if (topRect.width() > 0 && topRect.height() > 0) {
                    canvas.drawRect(topRect, hasFocus() ? mFocusPaint : mNoFocusPaint);
                }
                Rect bottomRect =
                    new Rect(viewDrawingRect.left, mDrawRect.bottom, viewDrawingRect.right,
            ...
            if (mMode == ModifyMode.Grow) {
                if (mCircle) {
                    ...
                    int d =
                        (int) Math.round(Math.cos(/* 45deg */Math.PI / 4D)
                            * (mDrawRect.width() / 2D));
